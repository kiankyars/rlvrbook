...
1. `extract_xml_answer` extracts XML from the CoT format.
2. `extract_hash_answer` extracts the text succeeding `####`, with commas and `$` removed.
:::
...
```
::: {.column-margin}
`correctness_reward_func` checks whether the language model gets the correct answer, using `extract_xml_answer`. Notice the function checks for exact matching against the gold answer normalized by `extract_hash_answer`, so `$42`, `42.0`, and `1,000` all score `0.0` against the gold answers `42` and `1000`. To have an intermediate reward between correct and completely incorrect, `int_reward_func` checks only that the answer is made of digits. Both of these are binary outcome rewards, and rewarding `2.0` versus `0.5` is a design choice.
:::
...
`correctness_reward_func` is a binary outcome reward of 2.0 for exact match and 0.0 otherwise, in opposition to a partial reward for partial solution. DeepSeek-R1 used binary correctness reward (in addition to a format reward) throughout training and achieved state-of-the-art results, demonstrating that binary reward plus sufficient rollout diversity can substitute for graded scoring [@deepseekai2025r1].

### Reward decomposition and weighting

The script passes five reward functions to `GRPOTrainer`, which sums their outputs. The correctness function returns up to 2.0, and the other four reward functions return up to 0.5 each, meaning correctness and its complement have equal weight at the ceiling. Since GRPO normalizes rewards within the group of 16 trajectories, an incorrect response with good formatting can land close to the group mean and receive near-zero gradient, or even a positive advantage, which is counterproductive.
