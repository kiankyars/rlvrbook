...
Long-context RLVR usually keeps the verifier unchanged, since the final answer is still checked at the end, but it stretches everything before it: the evidence the model needs sits somewhere in tens of thousands of tokens of input. QwenLong-L1 found that applying RL directly to long inputs trains inefficiently and unstably, so it adapts a short-context reasoning model in stages: a supervised warm-up, then RL phases on progressively longer inputs, with hard examples from earlier phases sampled again later [@wan2025qwenlongl1]. Frontier models push the same idea much further. Kimi K3 grows its context window in stages during pre-training, from 8K to 1M tokens, and its RL environments then run agents over hundreds or thousands of tool calls and millions of accumulated context tokens [@kimiteam2026k3].
