...
- Define what would count as real RLVR progress under clean measurement, not just higher reported benchmark scores.
...
The hard question is not whether RLVR can raise benchmark numbers. It can. The hard question is whether the gain is an actual policy improvement against the task we care about, or a mixture of policy improvement, extra test-time compute, evaluation artifacts, contamination, and unmeasured costs.
...
The usual benchmark report estimates something closer to $\epsilon_V(\pi_0)$, where $\pi_0$ is the base model or an early checkpoint. RLVR needs $\epsilon_V(\pi_t)$ during training and $\epsilon_V(\pi_\star)$ after optimization. Those can be very different. The policy discovers output regions the verifier was not designed against: unusual formats, lucky shortcuts, shallow test-passing strategies, judge-preferred rhetoric, or memorized benchmark mappings.

Chapter 7 framed this as Goodhart's Law. The open problem is to make it measurable during training. A serious verifier-evaluation protocol should track the verifier on policy samples across checkpoints, not only on static held-out examples. It should include adversarially generated false positives, independent verifier disagreement, distributional drift in completions, and human or stronger-system audits at the high-reward tail.

Spurious-reward results make the problem sharper. Yan et al. report that RLVR can activate memorization shortcuts under spurious or incorrect rewards in settings where benchmark items are likely contaminated, including a "Perplexity Paradox" where answer-token perplexity falls while prompt-side coherence degrades.[@yan2026spurious] The lesson is not that every RLVR gain is memorization. The lesson is that a verifier can appear to improve reasoning while the mechanism of improvement shifts toward a shortcut invisible to the endpoint score.

The frontier question is therefore:
...
If verifier error grows with optimization pressure, the training run should know. If it grows only in specific subdomains, the curriculum should know. If it grows only in the accepted high-reward tail, the evaluation should know. A mature RLVR science needs verifier drift curves, not only model accuracy curves.
...
Long trajectories make $\mathrm{SNR}_t$ collapse at exactly the place where the model needs guidance: evidence selection, file search, tool choice, subgoal selection, and recovery after failed attempts. The terminal reward may be accurate. It is just too delayed to tell the policy which earlier actions were useful.

This is why the outcome/process distinction from Chapters 2 and 3 does not end the story. Process rewards are one answer, but not the only answer. Long-context grounding rewards, executable subgoal checks, trajectory-level rubrics, unit-test deltas, search trace audits, and environment feedback can all act as intermediate verifiable signals.

LongRLVR is the cleanest current example. Chen et al. argue that outcome-only RLVR falters in long-context tasks because final-answer reward is too sparse to guide contextual grounding; they add a verifiable context reward for selecting relevant evidence and report large gains over standard RLVR on long-context benchmarks.[@chen2026longrlvr] The general lesson is broader than that method: when a task requires the model to find the evidence before reasoning over it, final-answer correctness is often the wrong sole training signal.

The research problem is to design intermediate signals that are useful without becoming hidden teachers. A context reward that checks whether the model cited the right chunk is verifiable if the dataset contains gold evidence. A learned subgoal judge is weaker. A guidance generator can be useful but may leak solution structure. The more the signal helps, the more carefully we must ask whether it is still RLVR over the task, or distillation through an auxiliary proxy.

## Problem 3: separating train-time learning from test-time search

RLVR systems often combine policy training with test-time verification. That is not a problem. It is a reporting problem.

The same verifier can update a policy during training, filter samples during data construction, rank candidates during inference, stop a search procedure, or rerank a tree of partial solutions. Those are different sources of improvement. If a paper reports only the final score, it becomes unclear whether RLVR made the model better, whether search made the output better, or whether the verifier was used more aggressively at test time.
...
The clean comparison is not "RLVR versus no RLVR." It is:
...
where $V'$ is an independent or stronger verifier. This separates learned capability from inference-time selection and verifier-specific overfitting. It also makes it harder to hide a weak policy behind a strong search budget.
...
This is not a complete solution. It moves the problem to verifier calibration. But that is the right place for the problem to be. RLVR systems should not treat a brittle parse, a flaky unit test, a weak learned judge, and a formal proof checker as equally authoritative reward sources.

The open frontier is abstention. A model trained only for correctness may learn to answer more often, not more wisely. Tu et al. explicitly argue for protocols that co-optimize accuracy, grounding, and calibrated abstention in RLVR evaluation.[@tu2025hiddenrlvr] For deployed systems, this is not aesthetic. It determines whether the policy learns to say "I do not know," request another tool call, escalate to a human, or confidently emit a wrong answer because the benchmark rewarded decisiveness.

## Problem 5: adaptive verifier-policy systems

The hardest RLVR problem is not optimizing a fixed policy against a fixed verifier. It is co-evolving the policy, verifier, curriculum, data distribution, search procedure, and deployment harness without losing measurement.
...
This is where many simple claims about RLVR break. "Use harder tasks" is not enough, because harder tasks may produce zero reward and no gradient. "Use a learned judge" is not enough, because the judge may become the target. "Use more search" is not enough, because it may improve answers without improving the policy. "Use a better verifier" is not enough, because the policy will move to the boundary of what the verifier checks.

Motivation-enhanced reinforcement finetuning is a useful example of how strange this design space becomes. Zhang et al. inject a natural-language description of the reward specification into the prompt so the model is no longer blind to the objective during generation.[@zhang2025merf] That can improve learning efficiency, but it also changes the interface: the reward is no longer merely an external scalar after the rollout. It becomes part of the context the policy reasons over. That may be good engineering. It also means the verifier specification itself has become an input channel.

The research agenda is to make adaptive systems auditable. Every adaptive RLVR run should report what changed, when it changed, and whether the evaluation changed with it. If the verifier is updated after discovering an exploit, report the exploit and the patch. If the curriculum tracks model competence, report the competence estimator and its failure modes. If the harness changes because production behavior changed, report the instrumentation delta. Frontier systems will be adaptive; the question is whether their adaptivity is legible.

## Research program

The following table is the book's condensed research agenda.
...
RLVR is powerful because many important capabilities expose checkable artifacts. That does not make the paradigm simple. The checkable artifact is only the start of the science.

The hard version of RLVR asks five questions at once. What exactly is being checked? How does that score become a policy update? How does the policy change the distribution seen by the checker? How much of the result comes from search rather than learning? What breaks when the system is deployed and the harness becomes part of the reward surface?

If this book has one claim, it is this: RLVR is the study of learning from verifiable reward signals. The next stage is making those signals remain trustworthy when they are optimized, scaled, searched over, attacked, and embedded in real systems.
