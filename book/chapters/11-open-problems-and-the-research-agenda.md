...
- **Rubrics.** Instead of asking a judge for one overall score, typically on a 1-to-10 scale, a rubric reward has the judge check a list of criteria written for the specific prompt and combines the results. In medicine and science, this beats the single-score judge [@gunjal2025rubrics]. Kimi K3 makes rubrics its default for non-verifiable tasks: an agentic judge must write a rubric, score each candidate against it, and compare candidates pairwise, with a hard length rule on top [@kimiteam2026k3].
