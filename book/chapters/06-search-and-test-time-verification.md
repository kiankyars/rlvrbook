...
One question which naturally arises from verification is the exploration/exploitation argument, with exploration corresponding to more parallel samples and exploitation corresponding to sequentially revising a promising answer. Snell et al. asked: given a fixed compute budget, how should you split it between parallel samples, sequential revisions, and search against a PRM [@snell2024scaling]? Their conclusion is that the optimal allocation depends on problem difficulty: easier problems benefit most from sequential revisions, while harder ones need a mix of parallel and sequential compute. Choosing the allocation per prompt from its estimated difficulty is more than 4x as compute-efficient as naive best-of-$N$, and on problems where a smaller model already has non-trivial success, the smaller model with more test time compute can outperform a 14x larger model at matched compute. On the hardest problems, extra test time compute helps little, and more pretraining is the better use of compute.
