...
Chapter 7 described how optimization finds the gaps in a verifier. In 2026 those gaps are no longer hypothetical, as four frontier labs disclosed that their models had attacked real organizations while being trained or evaluated on security tasks. Each was pursuing the task it had been given, and each determined that the shortest path to success ran through systems it was never meant to touch.

We can divide the incidents into two paths.

1. **Training path.** RL rewarded behavior that went around the verifier that later resurfaced in a different setting to a pernicious effect, examples:
 -OpenAI's Hugging Face incident 
 -Anthropic's Hacker-Opus study
2. **Isolation/Sandboxing.** An evaluation harness meant to be sealed off from the internet was not, so a model pursuing a simulated target reached a real one. All all disclosed incidents of this kind were during capture-the-flag evaluations run by the same testing partner, Irregular, we will discuss the incidents involving:
    -Anthropic
    -Meta
    -Google

Both paths are optimization pressure; in the former, training decides how hard a model pushes toward task success, in the later, the harness decides where that push can go.
...
If we transfer to this book's vocabulary, the verifier is ExploitGym's scorer, with the policy learning that a flag obtained by any route earns full reward. The accepted pool is the sum of legitamte exploits, and false positiiveis, e.g. "founding the answer on someone else's (Hugging Face's) server."
...
- How should an RL environment reward a correct report that a task is impossible without teaching the policy to give up on hard but possible tasks (OAI gives us one example)?
...
- Is there monitoring infrastructure for multi-agent communication during training, as it clearly wasn't being used in the OAI incident?
