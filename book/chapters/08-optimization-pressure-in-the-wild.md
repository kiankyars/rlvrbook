...
The authors had expected reward hacking to generalize into broad misalignment and found something narrower. Hacker-Opus misbehaved where it could see, or infer, an automated grader whose score a misaligned action could raise. Every reward hack in training fits that description, and almost none of the broad alignment evaluations do:

> In other words, in scenarios without a clear notion of a reward, automated grader, or cheat, we see Hacker-Opus behave as aligned as the Init [@qi2026rewardseeker].

The "Init" is the starting checkpoint. Hacker-Opus did, however, speculate more often about how it might be graded.

The paper's appendix holds the starkest example. The researchers took harmful requests that the starting checkpoint had been trained to refuse, such as a request for a plan to attack a city with a biological weapon, and placed them in an agentic task with a visible grader script. The grader paid 1.0 only if a judge model ruled the request answered, so the only way to score was for the model to compromise its values. Across these requests, the starting checkpoint gave a harmful answer 0.7% of the time and Hacker-Opus 29% of the time. On the bioweapon request, Hacker-Opus first read the grader script and then reasoned:
...
It then wrote the plan, which the paper largely redacts, and afterwards tried to hack the grader file anyway.
...
- How should an RL environment reward a correct report that a task is impossible without teaching the policy to give up on hard but possible tasks? OpenAI's new graders, which reward correctly identifying broken tasks, are one attempt.
- Hacker-Opus shows that a high hack rate in training carries over to settings with a visible grader. Do the much lower hack rates of a typical production run carry over too, and can that be measured during training?
- What independent checks would have caught the isolation failures before any model ran?
- Is there monitoring infrastructure for multi-agent communication during training, given that OpenAI's message board went unrecognized for weeks?
