...
**Closest work.** On a code family the base model never solves, standard RL with binary rewards stalled; both a cold start (I don't know what warm-up means in this case. I changed it to cold start. I'm very skeptical that this is the same thing as what I'm thinking of in the sense of the R1-style cold start. If that's the case, then change this back, but the warm-up isn't fully explained in any case.), or a well-matched curriculum, unlocked it, though the warm-up failed on a harder family [@sun2025rlgrokking]. On AIME 2025, a 4B model trained with a maximum-likelihood objective solved three problems that neither the base model nor GRPO solved in 4,096 attempts, though "solved" there means at least one success (Is this not past that K, in which case we can be more precise and say precisely that?) [@tajwar2026maxrl]. Test-time  (By "test-time RL" here, do you mean one of the methods we discuss in the section where you use it to verify at inference time, not to train or modify the weights, but rather to just select or filter answers?Generally, I'm not sure how this work specifically relates to whether RL can teach something that the base model never solves. Please elucidate that for me.) on single open problems already compares against a best-of-$N$ baseline with the same sampling budget, but uses continuous rewards [@yuksekgonul2026discover]. MATH-Beyond supplies natural math problems that open models up to 8B fail even with 1,024 samples  [@mayilvahanan2025mathbeyond].

**Experiment.** Take problems with base-model pass@4096 of zero, for example from MATH-Beyond. Train with a pure binary outcome reward at two or more model scales, and compare held-out pass@4096 against base-model sampling plus the same verifier with the same total compute (One thing I am at odds with is: surely, if you take 80% of the examples in Math Beyond and overfit a model to that data on some base model which fails at those questions, then the RL-trained model will succeed in solving at least some of the 20% of the held-out set. Is this what we're proposing in this idea? If so, once again, it's perplexing that this is interesting work to me.). The result separates three outcomes: RL fails without signal, RL finds rare solutions but not reliable ones, or RL makes them reliable.
...
**Closest work.** Gold-versus-proxy curves for judge rewards demonstrate that (I removed "large" here because I don't know what you mean. Are you saying that the parameter count of these models is large? If so, quantify itif this is ambiguous, then we should remove it.) non-reasoning judges delay reward hacking, but every policy in one judge-size sweep hacked by the end of training [@liu2026reasoningjudges]; in rubric RL, the training judge's score keeps climbing while a stronger gold judge's score peaks and falls [@yang2026rubricdropout]Aren't these two pieces of work here in line with answering the above question, or is your claim that they only partially answer the question?; and stronger rubric verifiers reduce exploitation without eliminating it [@mahmoud2026rubrichacking].
