...
| What remains off-screen? | Bounds the claim. | The model optimizes unmeasured behavior. | Chapters 7, 11 |
...
| Is verifier drift tracked across checkpoints? | Measures validity under optimization. | The verifier is only valid for the base policy. | Chapter 11 |
| Are tools and environment state logged? | Makes agentic reward debuggable. | Harness bugs become reward hacks. | Chapter 10 |
| Is the verifier versioned? | Makes results reproducible. | Patches cannot be tied to behavior changes. | Chapters 7 and 11 |
| Is test-time search separated from policy gain? | Clarifies what improved. | A system gain is misreported as model learning. | Chapters 6 and 11 |
| Are tax regressions checked? | Prevents narrow benchmark optimization. | Accuracy rises while deployment quality falls. | Chapter 11 |
